...
MCP server for Lendwise — read-only DeFi market comparison (Aave V3, Morpho, Compound V3). It compares markets; it never signs a transaction. Holds no secrets: it speaks only HTTPS to the public Lendwise API.
...
- `api/mcp.ts` — hosted Streamable HTTP via `mcp-handler`, deployed to Vercel. Public URL is `mcp.lendwise.fi/mcp`: Vercel mounts the function at `/api/mcp`, a rewrite in `vercel.json` maps `/mcp` onto it, and the function receives the *original* request path (`/mcp`, verified in prod), so `basePath` is `''` — mcp-handler matches the pathname exactly against `${basePath}/mcp`. Direct `/api/mcp` hits 308-redirect to `/mcp`. The rewrite, the redirect and `basePath` form one contract: change any of them together or every hosted request 404s while stdio still works.
